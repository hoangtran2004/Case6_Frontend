...
import {findJobById, getJob} from "../../service/Job-service";
...
function AuthOtherJob({otherJob}) {
    ...
    return (<div className="col-4">
        ...
        {otherJob && otherJob.map((item, index) => {
            ...
            if (job.companyId == item.companyId && item.jobId != jobId) {
                return (<Link to={`/job-detail/${item.jobId}`} onClick={() => {
                    dispatch(findJobById(item.jobId))
                }}>
